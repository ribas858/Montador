...
class Functions {
    ...
    public:
        ...
        static pair<int, int> has_line_instruction(ifstream& arq_entrada, map<string, Instrucao>& tab_inst, map<string, Diretiva>& tab_diret, map<string,
                                                    DiretivaExtend>& tab_extend, int begin_line, int cod);

        static void passagem1(map<string, Definicao>& tab_def, map<string, Simbolo>& tab_simbol, map<string,Instrucao>& tab_inst, map<string,Diretiva>&
                                                    tab_diret, vector<string>& linha, int& cont_posicao, int& line_count, int& flag_erros, map<string,
                                                    Plus>& tab_plus);

        static void passagem2(string& cod_obj, map<string, Simbolo>& tab_simbol, map<string,Instrucao>& tab_inst, map<string,Diretiva>& tab_diret, string&
        ...
        static void Analise_rot_instr(ifstream& arq_entrada, char& c, string& lexema, map<string, Instrucao>& tab_inst, map<string, Diretiva>& tab_diret,
